    ...
    <div className="invoice-list-container">
        ...
        <tbody>
          {invoices.map(invoice => (
            <tr key={invoice.id}>
              ...
              <td className="invoice-table-cell">{invoice.totalConsumption}</td>
              <td className="invoice-table-cell">
                <button className="view-invoice-btn" onClick={() => handleViewInvoice(invoice.id)}>View Invoice</button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {selectedInvoice && (
        <div className="invoice-details">
          ...
          <p className="invoice-details-item"><strong>Invoice ID:</strong> {selectedInvoice.id}</p>
          <p className="invoice-details-item"><strong>Bill Type:</strong> {selectedInvoice.billType}</p>
          <p className="invoice-details-item"><strong>Address:</strong> {selectedInvoice.address}</p>
          <p className="invoice-details-item"><strong>Billing Period:</strong> {selectedInvoice.billingPeriod}</p>
          <p className="invoice-details-item"><strong>Total Consumption:</strong> {selectedInvoice.totalConsumption}</p>
        </div>
      )}
    </div>
  );
